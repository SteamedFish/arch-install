...
- **btrfs journal NOCOW**: when the root filesystem is btrfs, `chattr +C` is applied to `/var/log/journal` at install time so journal files skip copy-on-write (avoids fragmentation of append-heavy logs)
