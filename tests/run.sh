...
check "dev-tools opencode/shellcheck 有 alarm 门禁(alarm 缺包)" "grep -q 'pkgs+=(opencode shellcheck)' modules/dev-tools.sh"
check "dev-tools 批次数组不含 shellcheck(已移入门禁)" "! grep -qE 'cmake shellcheck' modules/dev-tools.sh"
check "dev-tools biome 有 alarm 门禁(alarm 缺包)" "grep -q '|| pacman_install biome' modules/dev-tools.sh"
check "dev-tools LSP 批不含 biome(已移入门禁)" "! grep -qE 'uv biome' modules/dev-tools.sh"
check "cli-tools hwinfo/vi 有 alarm 门禁(alarm 缺包)" "grep -q '|| pacman_install hwinfo vi' modules/cli-tools.sh"
check "cli-tools 大列表不含 hwinfo/vi(已移入门禁)" "! grep -qE 'hwdata hwinfo' modules/cli-tools.sh && ! grep -qE 'at vi bat-extras' modules/cli-tools.sh"
